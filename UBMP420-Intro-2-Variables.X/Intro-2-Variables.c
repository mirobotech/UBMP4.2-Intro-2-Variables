 ...
 *    used for the SW2Count variable, only with the 'const' prefix added in its
 *    declaration. Can you think of some advantages of declaring a constant in
 *    this way, using a separate statement above the main code, rather than just
 ...
 *    LED D3. LED D3 should stay on continuously while the value of SW2Count is
 *    higher than maxCount. If LED D3 turns off, what can you infer about the
 ...
 *    during each cycle of the loop, and if SW2 is read as pressed, another
 ...
 *    words, it needs to respond to a *change* of SW2 state, from not-pressed
 *    to pressed. Doing this requires the use of another variable to store the
 *    prior state of SW2, so that its current state can be evaluated as being
 *    either the same, or different from its state in the previous loop. Replace
 *    the initial if-else condition with the following two if conditions:
 ...
 *    stored SW2Pressed variable so that new counts are only added when the SW2
 *    button is pressed while the SW2Pressed Boolean variable is false. In this
 *    case, a new switch press has been detected and the state of the Boolean
 *    variable is switched to true before checking and incrementing SW2Count.
 * 
 *    The following if structure resets the SW2Pressed Boolean to false when
 *    SW2 is released enabling the process to repeat. Try the code to verify
 *    that it works.
 * 
 ...
 * 
 * 7. A pushbutton's logic state can also be defined using a word, similar to a
 *    variable  declaration, which can help to make the code more readable. Add
 *    the following lines to the 'Program constant definitions' section at the
 ...
 *    switch logicl value in the definition statement, whereas before it was
 ...
 *    indicate that player 2 wins. Use a logical condition statement to reset
 *    the game by clearing bpth players counts as well as turning off the LEDs
 ...
 *    not a big concern, but switch bounce can be an issue in a microcontroller
 *    toggle button program because the speed of a microcontroller lets it see
 *    each bounce as a new, separate event. Use a variable to count the number
 *    of times a pushbutton is pressed and then display the count on the LEDs.
 *    Use a separate pushbutton to reset the count and turn off the LEDs so the
 *    test can be repeated. To determine if your switches bounce, try pressing
 *    them at various speeds and using different amounts of force.
 * 
